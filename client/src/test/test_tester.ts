    ...
    tests.addNestedTests('tester', testerTests => {
        testerTests.addTestWithNestedTests('run', run => {
            ...
            runTest.addTestWithNestedTests('withNestedTest', run => {
                ...
                withNestedTest.add('failingNestedTest', run => {
                ...
                withNestedTest.add('runWithoutNestedTests', run => {
                    let test: Entity = run.app.entity.createCode('foo', () => {
                    });
                    test.testG_installNestedTestsA();
                    test.testG_nestedTestsA.add_withoutApp('nestedTest', () => {
                    ...
                    let testRun: Entity = test.testG_run(true);

                    assert(testRun.testRunA.resultG_success);
                    assert_sameAs(testRun.testRunA.nestedRuns, undefined);
                });
            });
            runTest.add('failing', run => {
                let name = 'testName';
